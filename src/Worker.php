...
final class Worker
{
    ...
    /** @param array{host?: string, port?: int, token?: string, tls?: bool|array, batchSize?: int, pollTimeoutMs?: int, lockTtlMs?: int, heartbeatIntervalS?: float, name?: string, commandTimeout?: float} $options */
    public function __construct(public readonly string $queue, callable $processor, array $options = [])
    {
        ...
        $this->pollTimeoutMs = min($options['pollTimeoutMs'] ?? 5000, 30_000);
    ...
    /** SIGTERM/SIGINT -> graceful stop (finishes the in-flight job first). */
    ...
    private function emit(string $event, mixed ...$args): void
    {
        foreach ($this->listeners[$event] ?? [] as $listener) {
            try {
                ...
                // a throwing listener must never kill the pull loop
            }
        }
    }
}
